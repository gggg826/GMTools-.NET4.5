...
namespace CommLib.BdPush
{
    ...
    public class Push_All_Mod:Baidu_Mod
    {
        ...
        public Push_All_Mod(string apikey, string msg)
        {
            ...
            this.msg_type = 0;              //消息
            this.msg_expires = 7200;      //7天过期
            this.device_type = 3;           //安卓
            this.deploy_status = 2;         //生产状态
        }

        public Push_All_Mod(string apikey, string msg, Baidu_Helper.Device_Type dev_type, uint msg_type)
        {
            this.apikey = apikey;
            this.msg = msg;
            this.timestamp = Tool.getDefauleTimestamp();   //默认使用当前时间戳
            ...
            this.device_type = (uint)dev_type;           //设备类型
            this.deploy_status = 2;         //生产状态
        }
        /// <summary>
        /// 定时消息
        ...
        public Push_All_Mod(string apikey, string msg, uint msg_type, uint send_time)
        {
            this.apikey = apikey;
            this.msg = msg;
            this.timestamp = Tool.getDefauleTimestamp();   //默认使用当前时间戳
