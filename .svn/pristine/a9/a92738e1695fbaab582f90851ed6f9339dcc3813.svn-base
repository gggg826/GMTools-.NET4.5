...
namespace GameToolsHttpService
{
    public class GiftCodeModule : ProcessRequestBase
    {
        ...
        public override string Add(HttpContext context)
        {
            ...
            string strSessionID = context.Request["User"];
            ...
            try
            {
                ...
                for (int i = 0; i < iGiftCodeCount; i++)
                {
                    ...
                    uint giftCode = (uint)(FengNiao.GameToolsCommon.MD5.GetMD5String(DateTime.Now.ToString() + Guid.NewGuid().ToString()).GetHashCode());
                    string strGiftCode = SerialGiftCodeUtil.GetGiftCode(giftCode);
                    ...
                    giftCodeModel.fld_user = strSessionID;
                    try
                    {
    ...
    public class SerialGiftCodeUtil
    {
        ...
        public static string GetGiftCode(long num)
        {
            ...
            buf[--charPos] = ran[(int)(num % length)];
            string giftCode = new string(buf, charPos, (32 - charPos));
            if(giftCode.Length < finalLength)
            {
                StringBuilder sb = new StringBuilder();
                Random random = new Random();
                for (int i = 0; i < finalLength - giftCode.Length; i++)
                {
                    sb.Append(replenish[random.Next(24)]);
                }
                giftCode += sb.ToString();
            }
            return giftCode;
        }
    }
