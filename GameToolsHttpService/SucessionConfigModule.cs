...
namespace GameToolsHttpService
{
    public class SucessionConfigModule : ProcessRequestBase
    {
        ...
        private void Send(FengNiao.GMTools.Database.Model.tbl_server server, List<FengNiao.GMTools.Database.Model.tbl_sucession_rewards_config> models)
        {
            FengNiao.GMTools.Database.BLL.tbl_sucession_rewards_config ConfigBLL = new FengNiao.GMTools.Database.BLL.tbl_sucession_rewards_config();

            ConfigBLL.DeleteByServer(server.fld_server_id);
            if (models != null)
            {
                foreach (FengNiao.GMTools.Database.Model.tbl_sucession_rewards_config item in models)
                {
                    FengNiao.GMTools.Database.Model.tbl_sucession_rewards_config model = item;
