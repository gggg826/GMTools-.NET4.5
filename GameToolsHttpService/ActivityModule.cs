...
{
    public class ActivityModule : ProcessRequestBase
    {
        public ActivityModule(string tableName)
            : base(tableName)
        {
        }
        ...
        public override string Update(HttpContext context)
        {
            ...
            else
            {
                ...
                FengNiao.GMTools.Database.DAL.tbl_activity activityDAL = new FengNiao.GMTools.Database.DAL.tbl_activity();
                try
                {
                    activityDAL.Clear();
                    List<FengNiao.GMTools.Database.Model.tbl_activity> activitys = FengNiao.GameTools.Json.Serialize.ConvertJsonToObjectList<FengNiao.GMTools.Database.Model.tbl_activity>(strModel);
                    ...
                    for (int i = 0; i < activitys.Count; i++)
                    {
                        ...
                        try
                        {
                            if (activityDAL.Add(activitys[i]))
                            {
